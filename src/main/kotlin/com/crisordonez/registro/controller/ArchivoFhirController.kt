...
import com.crisordonez.registro.model.mapper.ArchivoFhirMapper.toFhirBundle
...
@RestController
@RequestMapping("/fhir/archivo")
class ArchivoFhirController(
    private val repo: ArchivoRepository
) {
    ...
    fun getOne(@PathVariable publicId: UUID): ResponseEntity<String> {
        val entity = repo.findByPublicId(publicId)
            .orElseThrow { ResponseStatusException(HttpStatus.NOT_FOUND, "Archivo no encontrado") }
